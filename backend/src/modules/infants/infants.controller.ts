import { Body, Controller, Delete, Get, HttpCode, HttpStatus, Post, Put, Query, Redirect, Req, Res } from '@nestjs/common';
...
export class InfantsController {
    ...
    @Get('params')
    @HttpCode(HttpStatus.OK)
    @ApiOperation({ summary: 'Obtener todos los infantes por parámetros' })
    @ApiResponse({ status: 200, description: 'Lista de infantes obtenida correctamente' })
    @ApiResponse({ status: 500, description: 'Error interno del servidor' })
    async getInfantsParams(@Query() query, @Res() res) {
        return this.infantsService.getInfantsParams(query, res);
    }
